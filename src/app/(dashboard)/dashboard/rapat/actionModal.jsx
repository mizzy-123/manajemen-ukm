...
import UpdateProker from "@/api/updateProker";

export default function ActionModal() {
  const { token, roleid, dataOrganisasi, dataProker, setDataProker } = useContext(AppContext);
      ...
      <div className="modal fade" id="modalEdit">
        <div className="modal-dialog" role="document">
          <div className="modal-content">
            ...
            <div className="modal-header">
              <h5 className="modal-title">Edit kegiatan/proker</h5>
              <a href="#" className="close" data-bs-dismiss="modal" aria-label="Close" ref={refSubmit}>
                <em className="icon ni ni-cross"></em>
              </a>
            </div>
            <div className="modal-body">
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    const response = await UpdateProker({ token: token.value, id: dataProker.id, name: name, lokasi: lokasi, date: tanggal, time: waktu });
                    if (response.status == 200) {
                      setMessage(response.data.message);
                      setAlertSuccess(true);
                      setAlert(false);
              ...
              >
                ...
                <div className="form-group">
                  ...
                  <div className="form-control-wrap">
                    <input
                      name="name"
                      value={dataProker.name}
                      type="text"
                      className={`form-control ${invalid && messageInvalid?.name?.[0] ? "is-invalid" : ""}`}
                      id="name"
                      onChange={(e) => {
                        setDataProker({ ...dataProker, name: e.target.value });
                      }}
                    />
                    {invalid && <div className="invalid-feedback">{messageInvalid?.name?.[0] !== undefined ? messageInvalid.name[0] : ""}</div>}
                  </div>
                ...
                <div className="form-group">
                  ...
                  <div className="form-control-wrap">
                    <input
                      name="lokasi"
                      value={dataProker.lokasi}
                      type="text"
                      className={`form-control ${invalid && messageInvalid?.lokasi?.[0] ? "is-invalid" : ""}`}
                      id="lokasi"
                      onChange={(e) => {
                        setDataProker({ ...dataProker, lokasi: e.target.value });
                      }}
                    />
                    {invalid && <div className="invalid-feedback">{messageInvalid?.lokasi?.[0] !== undefined ? messageInvalid.lokasi[0] : ""}</div>}
                  </div>
                ...
                <div className="form-group">
                  ...
                  <div className="form-control-wrap">
                    ...
                    <input
                      name="tanggal"
                      value={dataProker.date}
                      onChange={(e) => {
                        setDataProker({ ...dataProker, date: e.target.value });
                      }}
                      type="text"
                      className={`form-control date-picker ${invalid && messageInvalid?.tanggal?.[0] ? "is-invalid" : ""}`}
                ...
                <div className="form-group">
                  ...
                  <div className="form-control-wrap">
                    <input
                      name="waktu"
                      value={dataProker.time}
                      onChange={(e) => {
                        setDataProker({ ...dataProker, time: e.target.value });
                      }}
                      type="time"
                      className={`form-control ${invalid && messageInvalid?.waktu?.[0] ? "is-invalid" : ""}`}
